...
def init_centroids(num_clusters, image):
    ...
    # *** START YOUR CODE ***
    centroids_init = np.empty([num_clusters, image.shape[-1]])

    for i in range(num_clusters):
        rh = random.randint(0, image.shape[0])
        rw = random.randint(0, image.shape[1])
        centroids_init[i] = image[rh][rw]
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for i in range(max_iter):
        if i % print_every == 0:
            print(f"{i} iterations so far")
        for h in range(image.shape[0]):
            for w in range(image.shape[1]):
                min = float('inf')
                min_j = 0
                for j in range(centroids.shape[0]):
                    diff = np.linalg.norm(image[h][w] - centroids[j], 2) ** 2
                    if diff < min:
                        min = diff
                        min_j = j
                newImage[h][w] = min_j
        # inner loop to get new centroid values based on averages
        for j in range(centroids.shape[0]):
            numerator = np.zeros(centroids.shape[1])
            denominator = 0
            for h in range(image.shape[0]):
                for w in range(image.shape[1]):
                    if (newImage[h][w] == j):
